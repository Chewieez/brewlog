  ...
  const addBean = async (newBean: Omit<Bean, "id" | "createdAt">) => {
    ...
    try {
      const payload = {
        ...
        name: newBean.name,
        origin_country: newBean.originCountry || "Single Origin",
        region: newBean.region || null,
        farm: newBean.farm || null,
        variety: newBean.variety || [],
        altitude_meters: newBean.altitudeMeters || null,
        process: newBean.process || "washed",
        roast_level: newBean.roastLevel || "medium",
        roast_date: newBean.roastDate || new Date().toISOString().split("T")[0],
